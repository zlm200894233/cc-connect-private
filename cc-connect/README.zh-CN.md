...
- **可切换终端回复模式** — `/terminal mode text`、`/terminal mode screenshot`、`/terminal mode screenshot-progress` 分别对应最终文字、最终截图、过程截图加最终截图。
- **等待终端静止后再回复** — text 和 screenshot 模式都会等终端无动态刷新后再发最终结果，减少少回复/丢尾部结果，并过滤 Roosting、Context、进度帧等动态噪声。
...
| `/terminal mode text` | 等终端静止后发送最终文字。 |
| `/terminal mode screenshot` | 等终端静止后发送最终 PNG 截图。 |
